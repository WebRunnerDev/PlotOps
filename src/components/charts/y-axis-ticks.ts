...
export function resolveYAxisTickCount(numberTicks?: number): number {
    if (numberTicks == undefined || !Number.isFinite(numberTicks)) {
        return Y_AXIS_DEFAULT_TICK_COUNT;
    }
    const rounded = Math.round(numberTicks);
