...
export interface TimeSeriesChartInnerProperties {
...
export function TimeSeriesChartInner(
    properties: TimeSeriesChartInnerProperties
) {
    const { height, width } = properties;
    if (width < 10 || height < 10) {
        return null;
    }
    return <TimeSeriesChartCore {...properties} />;
}
...
function resolveTimeSeriesYDomain(
    ...
    const { maxValue, minValue } = collectNumericExtents(data, dataKeys);
...
const TimeSeriesChartCore = memo(function TimeSeriesChartCore({
    ...
    yScaleDomainMax,
}: TimeSeriesChartInnerProperties) {
        ...
        () =>
            computeYDomainsByAxis({
                ...
                resolveDomain: (dataKeys) =>
                    resolveYDomain(skeletonData, dataKeys),
            }),
    ...
    const animatedYDomainsByAxis = useAnimatedYDomains({
        ...
        tweenOnTargetChange:
            yDomainTween ||
            (tweenYDomainOnXDomainChange && xDomain != undefined),
    });
                ...
                <svg aria-hidden="true" height={height} width={width}>
                    <defs>
                        ...
                        {useClipReveal ? (
                            <ChartRevealClip
                                animating={
                                    isRevealAnimating || isRevealConcealing
                                }
