...
interface XAxisLabelProperties {
...
export function selectEvenlySpacedIndices(
    ...
    for (let tickCount = minCount; tickCount <= maxCount; tickCount++) {
        for (const rawIndices of allIndexLayouts(length, tickCount)) {
            ...
            const layoutScore = scoreTickLayout(
                indices,
                resolveXPx,
                targetCount
            );
...
export function XAxis(properties: XAxisProperties) {
    ...
    return <XAxisInner {...properties} container={container} />;
}
...
function appendProjectionTailTicks(
    ...
    for (let index = 1; index <= extraCount; index++) {
        const date = new Date(
            startTime + (index / (extraCount + 1)) * (endTime - startTime)
        );
...
function buildDataAlignedTicks({
    ...
    for (const index of selectEvenlySpacedIndices(
        data.length,
        targetTickCount,
        {
            data,
...
const XAxisInner = memo(function XAxisInner({
    container,
    numTicks: numberTicks = 5,
    tickerHalfWidth = 50,
    tickMode = "data",
}: XAxisProperties & { container: HTMLDivElement }) {
    const {
        data,
        dateLabels,
        margin,
    ...
    const labelsToShow = useMemo(() => {
        ...
        if (projectionExtendsScale && xDomain == undefined) {
            return buildDomainTicks({
                marginLeft: margin.left,
                numTicks: numberTicks,
                xScale,
        ...
        if (projectionExtendsScale && xDomain != undefined) {
            return appendProjectionTailTicks(
                ...
                xScale,
                margin.left,
                Math.max(1, numberTicks - dataTicks.length + 1)
            );
        }
    ...
    return createPortal(
        <div className="pointer-events-none absolute inset-0">
            {labelsToShow.map((item) => (
                <XAxisLabel
                    animatePosition={xDomain == undefined}
