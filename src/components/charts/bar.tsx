...
export type BarAnimationType = "fade" | "grow";

export type BarLineCap = "butt" | "round" | number;
...
interface AnimatedBarProperties {
    animationType: BarAnimationType;
...
interface BarInnerProperties extends BarProperties {
    bandWidth: number;
            ...
            {data.map((d, index) => {
                ...
                if (isHorizontal) {
                    ...
                    if (stacked && stackOffsets) {
                        const offset =
                            stackOffsets.get(index)?.get(dataKey) ?? 0;
                        x = scale(offset) ?? 0;
                        barW = valuePos - x;
...
export function Bar(properties: BarProperties) {
