...
export function usePathStrokeMetrics(
    pathReference: RefObject<null | SVGPathElement>,
    deps: readonly unknown[]
): PathStrokeMetrics {
    const [metrics, setMetrics] = useState<PathStrokeMetrics>(EMPTY_METRICS);

    useEffect(() => {
        const path = pathReference.current;
        if (!path) {
            return;
        }
        const d = path.getAttribute("d");
        const length_ = d ? path.getTotalLength() : 0;
        setMetrics((previous) =>
            previous.pathD === d && previous.pathLength === length_
                ? previous
                : { pathD: d, pathLength: length_ }
        );
    }, deps);
