...
interface ChartInteractionResult {
    ...
    setTooltipData: React.Dispatch<React.SetStateAction<null | TooltipData>>;
...
interface UseChartInteractionParameters {
    bisectDate: (
...
export function useChartInteraction({
    ...
    yScale,
    yScales,
}: UseChartInteractionParameters): ChartInteractionResult {
    ...
    const dragStartXReference = useRef<number>(0);
    const lastHoveredXReference = useRef<null | number>(null);

    const resolveTooltipFromX = useCallback(
        (pixelX: number): null | TooltipData => {
    ...
    const getChartX = useCallback(
        (
            event:
                React.MouseEvent<SVGGElement> | React.TouchEvent<SVGGElement>,
            touchIndex = 0
        ): null | number => {
    ...
    const handleMouseMove = useCallback(
        (event: React.MouseEvent<SVGGElement>) => {
            ...
            if (isDraggingReference.current) {
                const startX = Math.min(dragStartXReference.current, chartX);
                const endX = Math.max(dragStartXReference.current, chartX);
                setSelection({
                    active: true,
            ...
            lastHoveredXReference.current = chartX;
    ...
    const handleMouseLeave = useCallback(() => {
        lastHoveredXReference.current = null;
        clearTooltip();
        if (isDraggingReference.current) {
    ...
    const handleMouseDown = useCallback(
        (event: React.MouseEvent<SVGGElement>) => {
            ...
            dragStartXReference.current = chartX;
            clearTooltip();
            setSelection(null);
        },
    ...
    useEffect(() => {
        if (!canInteract || lastHoveredXReference.current === null) {
            return;
        }
        const tooltip = resolveTooltipFromX(lastHoveredXReference.current);
        if (tooltip) {
            scheduleTooltip(
                tooltip,
