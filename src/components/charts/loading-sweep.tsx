...
export interface LineLoadingSweepProperties {
...
export function getSkeletonHeights(
    ...
    return Array.from(
        { length: count },
        (_, index) =>
            min + Math.floor(hashFract((index + 1) * 12.9898 + seed) * range)
    );
}
...
export function LineLoadingSweep({
    ...
    stroke = chartCssVariables.foreground,
    strokeOpacity = LINE_STROKE_OPACITY,
    strokeWidth = 2,
    withArea = false,
}: LineLoadingSweepProperties) {
    const { innerHeight, innerWidth } = useChartStable();
    const reduceMotion = useReducedMotion();
    const reactId = useId();
    const chartId = `line-sweep-${reactId.replaceAll(/[^a-zA-Z0-9_-]/g, "")}`;
...
function getSkeletonSigns(count: number, seed = 0): number[] {
    return Array.from({ length: count }, (_, index) =>
        hashFract((index + 1) * 78.233 + seed) < 0.5 ? -1 : 1
    );
}
...
function LoadingSweepMask({
    ...
    const lastXReference = useRef(SWEEP_START_X);

    const handleUpdate = useCallback(
        (latest: { x?: number }) => {
            const xValue =
                typeof latest.x === "number" ? latest.x : SWEEP_START_X;
            ...
            if (xValue >= 1 && lastXReference.current < 1) {
                onSweepComplete();
            }
            lastXReference.current = xValue;
        },
        [onSweepComplete]
    );
...
export interface BarLoadingSkeletonProperties {
...
export function BarLoadingSkeleton({
    ...
    innerHeight,
    innerWidth,
}: BarLoadingSkeletonProperties) {
    const reduceMotion = useReducedMotion();
    const reactId = useId();
    const chartId = `bar-sweep-${reactId.replaceAll(/[^a-zA-Z0-9_-]/g, "")}`;
    const [tick, setTick] = useState(0);
    const onSweepComplete = useCallback(
        () => setTick((previous) => previous + 1),
        []
