...
describe("board multi-select archive seams", () => {
    it("wires selection store, bar, and card checkbox stopPropagation", () => {
        ...
        expect(card).toMatch(/selectionActive/);
        expect(board).toMatch(/clearMultiSelectionAfterDrop/);
        ...
        expect(card).toMatch(/selectRangeInColumn/);
        expect(card).toMatch(/isBoardMultiSelectModifier/);
        expect(card).toMatch(/event\.shiftKey/);
        expect(card).toMatch(/shouldPreventBoardTaskTextSelection/);
        expect(card).toMatch(/select-none/);
        expect(card).toMatch(/onDoubleClick/);
        expect(column).toMatch(/columnTaskIds=/);
