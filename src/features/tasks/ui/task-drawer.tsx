...
import { resolveTaskDrawerPlacement } from "@/features/tasks/lib/resolve-task-drawer-placement";
...
export function TaskDrawer({
    ...
    const sideDrawerWidthPx = useTaskDrawerPreferencesStore(
        (state) => state.sideDrawerWidthPx
    );
    ...
    const { data: mentionProjectTasks = [] } = useProjectTasks(
        projectId,
    ...
    const clearTaskSwap = useTasksUiStore((state) => state.clearTaskSwap);
    const swapFromTask =
        taskSwapFromId == undefined
            ? undefined
            : (tasks.find((item) => item.id === taskSwapFromId) ??
              archivedTasks.find((item) => item.id === taskSwapFromId) ??
              mentionProjectTasks.find((item) => item.id === taskSwapFromId));
    const {
        direction: taskSwapDirection,
        shouldAnimate: shouldAnimateTaskSwap,
    } = resolveTaskSwapAnimation({
        currentId: task?.id,
        currentParentId: task?.parentId,
        previousId: taskSwapFromId,
        previousParentId: swapFromTask?.parentId,
    });
    ...
    useEffect(() => {
        if (!shouldAnimateTaskSwap || taskSwapEpoch === 0) return;
    ...
    }, [clearTaskSwap, shouldAnimateTaskSwap, taskSwapEpoch]);
    ...
    useEffect(() => {
        ...
        void navigate({
            ...
            search: selectedTaskId ? { task: selectedTaskId } : undefined,
    ...
    const handleCopyTaskLink = async () => {
        ...
        try {
            ...
            toast.success(t("fields.copiedTaskLink"));
            globalThis.setTimeout(() => setCopiedTaskLink(false), 1500);
        } catch {
    ...
    const handleCopyTaskText = async () => {
        ...
        const plain = formatTaskCopyText(sections);
        const html = formatTaskCopyHtml(sections);
        ...
        toast.success(t("fields.copiedTaskContent"));
    ...
    return (
        <>
            ...
            >
                <DrawerContent
                    className={drawerPlacement.contentClassName}
                    style={drawerPlacement.contentStyle}
                >
                    {task ? (
                        <div
                            className={cn(
                        ...
                        >
                            ...
                            <div className="flex min-h-0 min-w-0 flex-1 flex-col">
                                ...
                                >
                                    <div className="mx-auto grid w-full min-w-0 max-w-7xl grid-cols-1 gap-6 @3xl/task-drawer:grid-cols-[minmax(0,2fr)_auto_minmax(0,1fr)] @3xl/task-drawer:items-stretch @3xl/task-drawer:gap-8">
                                        ...
                                        <Separator
                                            className="hidden @3xl/task-drawer:block"
