...
| Git integration (PR, diff, branches)                          | ✅ Done (Git tab; branch generate/link/skip; link PR; in-app code diff; Open PR + Merge + Close from task panel — ADR 0022; PR check runs rollup in drawer — ADR 0025; Approve / request-review still deferred)                                                                                                                                                                                                                                                                                                     |
...
| Multi-board + branch mapping                                  | ✅ Done (ADR 0006; Boards under Project; Base branch + Allowed patterns; soft warn; default task type; auto-assign to creator on solo Team)                                                                                                                                                                                                                                                                                                                                                                         |
...
| In-app Approve / request review      | Open PR + Merge + Close shipped (ADR 0022). Approve and request-review remain parked.                                                                                                                                                      |
...
- **Open PR + Merge + Close in-app** (ADR 0022): from the task GitHub panel via the user’s `provider_token` (`repo` scope). Base = Board base branch; head = task branch. PlotOps gate: Owner/Admin any task; Manager/Contributor only if author or assignee; guests hidden. Approve / request-review stay deferred. After merge (in-app or on GitHub), `github-webhook` moves the linked Task to the Board’s **last column** when the PR merges into that Board’s **Base branch** (webhook remains source of truth for column). Non-merge PR close (in-app or webhook) sets `pr_state=closed` only — no column move.
...
- Open PR + Merge + Close from task GitHub panel (ADR 0022); Approve remains deferred.
