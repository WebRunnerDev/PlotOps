...
| Team & permissions (`project_members`, roles, invites) | ✅ Done (Project-boundary MVP); 🟡 Team above Project — domain locked ADR 0017; schema+RLS #155 done (local); UI/gating tickets open                                                   |
...
4. **Team settings** — members, invites, leave/remove, Owner transfer (lifted semantics).
5. **Invite redeem** — `/invite/$token` joins a Team.
6. **Project lifecycle** — create under Team (GH connect), delete Project (Owner), delete empty Team.
7. **UI gating** — `useTeamAccess` for Board/Git/settings controls (RLS remains source of truth).
