...
| Team & permissions (`project_members`, roles, invites) | ✅ Done (Project-boundary MVP); 🟡 Team above Project — domain locked ADR 0017; schema+RLS #155 done (local); Team settings #157 + invite redeem #158 done (local); UI gating #160 done (Board/Git/settings + thin wrap; create-under-Team / delete-Team UI with Home/lifecycle tickets); Home/lifecycle tickets open |
...
3. **Home / Team shell** — list Teams, create Team, Team projects list.
...
6. **Project lifecycle** — create under Team (GH connect), delete Project (Owner), delete empty Team.
7. **UI gating** — `useTeamAccess` for Board/Git/settings controls (RLS remains source of truth). ✅ (#160; create-under-Team / delete empty Team affordances land with Home/lifecycle UI)
