...
| Custom text fields (Project-scoped)                           | 🟡 Schema + RLS + API/hooks done (ADR 0024); Settings UI + Task drawer still open — ≤10 text defs; `applies_to`; transfer copy; Guest stub seed                                                                                                                                                                                                                                                                                                                                                                     |
...
| `custom_field_definitions` | `id`, `project_id`, `name`, `position`, `applies_to` (`task_type[]`, ≥1), `created_at` (≤10 per Project; drawer filters by Task type — ADR 0024; schema shipped)                                                                             |
| `task_custom_field_values` | `(task_id, field_id)` PK, `value` text (cascade on definition or task delete; kept across Task type changes; schema shipped)                                                                                                                 |
