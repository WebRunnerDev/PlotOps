...
- **MVP type is text only.** Add / rename / reorder / delete definitions and edit `applies_to` in Project Settings (Manager+). Cap **≤10** definitions per Project (DB-enforced). Values edited in the Task drawer (Contributor+), not shown as Kanban card chips in MVP.
