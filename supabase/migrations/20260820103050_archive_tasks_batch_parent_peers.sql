-- Bulk archive: Parent + its Subtasks in the same batch is legal.
-- Archive Subtasks first so tasks_archive_guard sees them as archived
-- before the Parent row is updated. Active Subtasks outside the batch
-- still block Parent archive.
...
begin
  ...
  if exists (
    ...
    where parent.id = any (p_task_ids)
  ...
  -- Phase 1: Subtasks in the batch (so Parent trigger sees them archived).
  with updated as (
  ...
  v_archived_ids := v_archived_ids || v_phase_ids;

  -- Phase 2: remaining selected tasks (Parents and root Tasks).
  with updated as (
    update public.tasks as t
    set archived_at = now()
    where t.id = any (p_task_ids)
