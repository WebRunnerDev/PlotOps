-- Optional manually linked commit SHA + smart-commit discovery by task key in message.

alter table public.tasks
  add column if not exists linked_commit_sha text null;

comment on column public.tasks.linked_commit_sha is
  'Optional pinned commit SHA. Commits whose message contains task_key are also discovered via GitHub search.';
