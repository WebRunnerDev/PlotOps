-- ADR 0030: Close carryover may target another Active Sprint on the same Board
-- (as well as Draft / Backlog). Cannot carryover into the Sprint being closed.
...
begin
  ...
  loop
    ...
    if carry_row.id = p_sprint_id then
      raise exception 'Carryover target cannot be the sprint being closed'
        using errcode = '23514';
    end if;

    if carry_row.state not in ('draft', 'active') then
      raise exception 'Carryover target must be a draft or active sprint'
        using errcode = '23514';
    end if;
