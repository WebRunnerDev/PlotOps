-- Per-task carryover on Close: map task_id → carryover_sprint_id | null
-- (null / missing key = Backlog). Replaces single p_carryover_sprint_id.

drop function if exists public.close_sprint(uuid, uuid[], uuid);
...
revoke all on function public.close_sprint(uuid, uuid[], jsonb) from public;
grant execute on function public.close_sprint(uuid, uuid[], jsonb) to authenticated;
