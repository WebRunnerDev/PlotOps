-- update_task_details used `patch ->> 'task_type'` (text) in a CASE that also
-- returns `t.task_type` (public.task_type). Postgres rejects that with 42804
-- "CASE types public.task_type and text cannot be matched" on any details
-- update that touches the row (even when task_type is not in the patch — the
-- CASE expression is still type-checked).
